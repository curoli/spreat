...
function hexagonPoints(cxy) {
  ...
  for (var ic = 0; ic < 6; ic++) {
    ...
    xyStrings.push(
      cxy.x + ry * Math.cos(angle) + "," + (cxy.y + ry * Math.sin(angle)),
    );
  }
...
function initFields() {
  ...
  var fieldShapes = d3
    .select("svg")
    .attr("width", 2 * pad + rx * (4 * boardSize + 2))
    .attr("height", 2 * pad + rx + ry * (3 * boardSize + 1.5));
}
...
function refreshAtomShapes() {
  var atomShapes0 = d3.select("svg").selectAll("circle").data(atoms);
  atomShapes0.enter().append("circle");
  atomShapes = atomShapes0.transition().duration(0);
}

function drawAtoms(duration) {
  atomShapes = atomShapes
    .transition()
    .duration(duration)
    .attr("cx", function (d) {
      return d.x;
  ...
  while (keepGoing && nDistributions <= nDistributionsMax) {
    ...
    for (var iField = 0; iField < fields.length; iField++) {
      ...
      if (field.atoms.length >= field.iNeighbours.length) {
        for (
          var iFieldAtom = 0;
          iFieldAtom < field.iNeighbours.length;
          iFieldAtom++
        ) {
